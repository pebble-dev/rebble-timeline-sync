...
import uuid
import requests
from .models import db, SandboxToken, TimelinePin, UserTimeline, TimelineTopic, TimelineTopicSubscription, AppGlance, FcmToken
...
@api.route('/user/fcm_token/<token>', methods=['PUT', 'DELETE'])
def fcm_token(token):
    user_id = get_uid()
